...
{
    assert(iSocketType == SOCK_STREAM || iSocketType == SOCK_DGRAM); //judgment
    close();
...
void TC_Socket::bind(const string &sServerAddr, int port)
{
    ...
    if (sServerAddr == "")
    {
        bindAddr.sin_addr.s_addr = htonl(INADDR_ANY); //all networkcards
    }
    else
    {
...
{
    if (_sock != INVALID_SOCKET) //无效socket
    {
        ::close(_sock);
        _sock = INVALID_SOCKET;
    }
