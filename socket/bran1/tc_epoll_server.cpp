...
int NetThread::bind(string& ip, int& port)
{
    ...
    bzero(&bindAddr, sizeof(bindAddr)); //0
    ...
    setsockopt(_sock, SOL_SOCKET, SO_REUSEADDR, (const void *)&iReuseAddr, sizeof(int)); //true: reutrn 0, error: return -1
	
    if(::bind(_sock, (struct sockaddr *)(&bindAddr), sizeof(bindAddr)) < 0) //全局
    {
    ...
    int flag = 1;
    if(setsockopt(_sock, SOL_SOCKET, SO_KEEPALIVE, (char*)&flag, int(sizeof(int))) == -1) //重用该socket  SO_KEEPALIVE: keep connect
    {
        cout<<"setKeepAlive] error"<<endl;
    }

    flag=1;
	if(setsockopt(_sock, IPPROTO_TCP, TCP_NODELAY, (char*)&flag, int(sizeof(int))) == -1)  //IPPROTO_TCP: TCP, TCP_NODELAY 不使用Nagle算法
	{
	...
	linger stLinger;
	/*
	 * https://blog.csdn.net/weixin_33939843/article/details/92429784
	 */
	stLinger.l_onoff = 1;  //在close socket调用后, 但是还有数据没发送完毕的时候容许逗留
	stLinger.l_linger = 0; //容许逗留的时间为0秒

	if(setsockopt(_sock, SOL_SOCKET, SO_LINGER, (const void *)&stLinger, sizeof(linger)) == -1) //SO_LINGER: keep 0/s
	{
	...
	if ((val = fcntl(_sock, F_GETFL, 0)) == -1) //关闭fd的close-on-exec标志
	{
...
{
	//将点分十进制转为二进制整数
	int iRet = inet_pton(AF_INET, sAddr.c_str(), &stSinAddr); //sAddr to stSinAddr
	if(iRet < 0)
	{
	...
	else if(iRet == 0)
	{
		...
		gethostbyname_r(sAddr.c_str(), &stHostent, buf, sizeof(buf), &pstHostent, &iError); //gethostbyname_r()调用成功时返回0, *result指向解析成功的数据结构, *result如果为NULL则表示解析出错
...
void NetThread::createEpoll(uint32_t iIndex)
{
    ...
    _epoller.add(_shutdown_sock, H64(ET_CLOSE), EPOLLIN); //EPOLLIN: 表示对应的文件描述符可以读（包括对端SOCKET正常关闭)
    
    _epoller.add(_notify_sock, H64(ET_NOTIFY), EPOLLIN);

	_epoller.add(_sock, H64(ET_LISTEN) | _sock, EPOLLIN);	

	for(uint32_t i = 1; i <= _total; i++) //_total = 200000
	{
		_free.push_back(i);
